...
from BEM_Utils import load_wamit_3
...
class SSFit_Excitation(object):
    ...
    def __init__(self, **kwargs):
        ...
        super(SSFit_Excitation, self).__init__()
    ...
    def readInputs(self):
        ...
        with open(self.InputFile,'r') as f:
            try:
                ...
                self.InputVars['HydroFile']     = f.readline().split('%')[0].strip()        
                self.InputVars['DOFs']          = np.fromstring(f.readline().split('%')[0].strip()[1:-1],dtype=int,sep = ' ')
                self.InputVars['WaveTMax']      = float(f.readline().split('%')[0])
                self.InputVars['dT']            = float(f.readline().split('%')[0])
                self.InputVars['FitReq']        = float(f.readline().split('%')[0])
                self.InputVars['PlotFit']       = bool(f.readline().split('%')[0])
                self.InputVars['ManOrdRed']     = bool(f.readline().split('%')[0])
                self.InputVars['MaxIter']       = int(f.readline().split('%')[0])
                self.InputVars['WtrDen']        = float(f.readline().split('%')[0])
                self.InputVars['Grav']          = float(f.readline().split('%')[0])
                self.InputVars['WaveDir']       = float(f.readline().split('%')[0])
                self.InputVars['OutFile']       = f.readline().split('%')[0].strip()
    ...
    def causalIRF(self,wamData,Plot=True):
        # Upsample FRF
        dt      = self.InputVars['dT']
        Tmax    = self.InputVars['WaveTMax']
        dw      = 2 * np.pi / Tmax

        ww      = np.arange(0,np.pi/dt+dw,dw)

        K_w     = interpolate.interp1d(wamData.omega,wamData.K_dir,axis=0,bounds_error=False,fill_value=0)
        K       = K_w(ww)
        ...
        tc  = 12.                   # time to shift by
        print('Using tc = {:.4f} sec.'.format(tc))

        Nc  = math.ceil(tc/dt)        # samples to shift by
        ...
        tt_c    = tt[startInd:startInd+nSampOut] + tc
        ...
        # Shift Impulse response function

    def writeMats(self,sys_fit):
        ...
        # write output files
        with open(self.InputVars['HydroFile']+'_py.ssexctn','w') as f:

            # header info
            now = datetime.now()
            print('here')
            f.write('{}\n'.format('SS_Fitting v1.00.01: State-Spaces Matrices obtained using Time Domain method, on ' + now.strftime('%b-%d-%Y %H:%M:%S')))
            f.write('{}\t\t\t\t\t\t\t{}\n'.format(self.InputVars['WaveDir'],'%Wave heading angle'))
            f.write('{}\t\t\t\t\t\t{}\n'.format(self.t_c,'%Enabled DoFs'))
...
class WAMIT_Out(object):
    ...
    def readFRFs(self,dir=[],grav=[],dens=[]):
        ...
        if self.Type == 1:
            ...
            # Read WAMIT outputs
            wamOuts = load_wamit_3(self.HydroFile+'.3')

            # frequency
            omega   = 2* np.pi / np.array(wamOuts['periods_diff'])
            ...
            fDir    = interpolate.interp1d(wamOuts['direction_diff'],Kbar,axis=1)
            Kbar_dir = fDir(dir)
...
class TimeDomain_Fit(object):
    ...
    def __init__(self,Kt,tt,fitReq=0.99,**kwargs):
        ...
        super(TimeDomain_Fit, self).__init__()


    def fit(self,Plot=False,verbose=False):
        ...
        for iDOF, Ki in enumerate(np.transpose(self.Kt)):
            ...
            if Plot:
                ...
                plt.plot(t_lo,y_lo,label='Low order IRF, {} states'.format(K_lo.states))
                plt.xlabel('Time (s)')
