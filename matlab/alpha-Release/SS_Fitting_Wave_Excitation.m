...
fields = {'FileName','WaveTMax','dT','Fit','ppmf','fmt','maxiter', 'WtrDen', 'Grav','WaveDir','OutFileName'}; %Local structure Excite
try
Excite=cell2struct(Excite{1}([1 3:12]),fields);
...
% (.3) data and forms the frequency response function.
...
% change made to correct indexing 5-15-2020:

        if length(HdroWvDir) == 1  %only one direction defined in WAMIT.3 file
            if HdroWvDir ~= Excite.WaveDir
                disp(['WARNING: only one WaveDir specified in ',strcat(Excite.FileName,'.3')])
                disp('and its not equal to Excite.WaveDir, processing anyway as if it were');
            end
            
            KcDir(jjj,1,iii) = HdroExctn(jjj,1,iii);
            
        else
            KcDir(jjj,1,iii) = interp1(HdroWvDir,HdroExctn(jjj,:,iii),Excite.WaveDir,'linear','extrap');
        end
% end change
     end
   end
...
% change made to set Kc1 equal to the interpolated value for wave direction:
%Kc1 = HdroExctn(:,1,:);
   Kc1 = KcDir;
