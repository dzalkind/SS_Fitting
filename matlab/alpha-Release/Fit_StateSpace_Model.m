...
%    windowSize = 8;
    ...
    if Excite.fmt==0 %If Automatic order reduction is selected
        ...
        Ny = length(y);
        Nmin = min(NKt,Ny);
        p1=plot(plottime(1:Nmin),KtSmooth(1:Nmin,dof(s)),t(1:Nmin),y(1:Nmin),'Linewidth',2);
        Legend = cell(2,1);
...
%        legend('X(t)','~Kt(t)-original (i=200)');
        ...
        while a==0 %Is the user satisfied with the goodness of the fit
            ...
            plotY(iii)=plot(T,Y,'Linewidth',2);
            %plot(plottime,Y,'Linewidth',2);
            Legend{iii+2}=strcat('',num2str(Nn(iii)),' th order ss approx');
            legend(Legend{1:iii+2});
