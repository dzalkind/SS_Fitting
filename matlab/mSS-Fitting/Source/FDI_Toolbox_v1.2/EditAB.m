function [Amod,Bmod,Wmod]=EditAB(A,B,W,wmin,wmax)
...
%     xlabel('Frequency [rad/s]')
%     display('---------------------------------------------------------------------')
%     display('Click on the plot twice to define low and high frequency, then Enter.')
%     FR=ginput;
%     FR=[0;5];
    %
    Wmin = wmin;%FR(1,1);
...
%     display('---------------------------------------------------------------------')
%     Usropt=input('Do you need to eliminate wild points? (y,n):','s');
    Usropt='n';
    if Usropt == 'y',
        FR=ginput;
        %
    ...
    figure(101)
        ...
        xlabel('Frequency [rad/s]')
% display('---------------------------------------------------------------------')
Usropt=input('Start again with range selection an wild point elimination? (y,n):','s');
if Usropt=='n'
    break
end
end
